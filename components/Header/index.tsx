...
export default function Header() {
  ...
  // Debouncing search functionality to improve performance
  const debounceSearch = useCallback(
    debounce((nextValue: string) => {
      dispatch(setKeyword(nextValue)); // Dispatching action to update keyword in Redux store
    }, 500),
    []
  );
        ...
        <div className="w-1/3 mt-4 h-full flex ml-4 items-center">
          ...
          <div className="h-full">
            <input
              ...
              onChange={(e) => {
                debounceSearch(e.target.value); // Debouncing the search input
              }}
            />
          </div>
        </div>
